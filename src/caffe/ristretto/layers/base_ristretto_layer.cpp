...
//quantized with blob pointer
template <typename Dtype>
void BaseRistrettoLayer<Dtype>::QuantizeWeights_cpu(Dtype* weight,
      const int count, const int rounding ,const char* type_name, const int blob_dim) {
  int bw_temp=32;
  int fl_temp=16;
  if (strcmp(type_name, "ConvolutionRistretto") == 0 || 
        strcmp(type_name, "DeconvolutionRistretto") == 0 || 
        strcmp(type_name, "FcRistretto") == 0 ||
        strcmp(type_name, "ScaleRistretto") == 0 ||
        strcmp(type_name, "BiasRistretto") == 0) {
    bw_temp = bw_params_;
    fl_temp = fl_params_;
  }
  if (strcmp(type_name, "BatchNormRistretto") == 0 ) {
    switch (blob_dim) {
      case 0: {
        bw_temp = bw_mean_;
        fl_temp = fl_mean_;
      ...
      default:
        LOG(FATAL) << "Unknown trimming blob_dim: " << blob_dim;
        break;
    }
  }
  switch (precision_) {
    // case QuantizationParameter_Precision_INTEGER_POWER_OF_2_WEIGHTS:
    //   Trim2IntegerPowerOf2_cpu(weight, count, pow_2_min_exp_, pow_2_max_exp_,
    //     rounding);
    //   break;
    case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
      Trim2FixedPoint_cpu(weight, count, bw_temp, rounding, fl_temp);
      break;
    // case QuantizationParameter_Precision_MINIFLOAT:
    //   Trim2MiniFloat_cpu(weight, count, fp_mant_, fp_exp_, rounding);
    //   break;
    default:
      LOG(FATAL) << "Unknown trimming mode: " << precision_;
      break;
