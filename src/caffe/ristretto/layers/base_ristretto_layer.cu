...
void BaseRistrettoLayer<Dtype>::QuantizeWeights_gpu(Dtype* weight,
  ...
  switch (precision_) {
    //case QuantizationParameter_Precision_INTEGER_POWER_OF_2_WEIGHTS:
    //  Trim2IntegerPowerOf2_gpu(weight, count, pow_2_min_exp_, pow_2_max_exp_,
    //    rounding);
    //  break;
    case QuantizationParameter_Precision_DYNAMIC_FIXED_POINT:
      Trim2FixedPoint_gpu(weight, count, bw_temp, rounding, fl_temp);
      break;
  //  case QuantizationParameter_Precision_MINIFLOAT:
  //    Trim2MiniFloat_gpu(weight, count, fp_mant_, fp_exp_, rounding);
  //    break;
    default:
      LOG(FATAL) << "Unknown trimming mode: " << precision_;
      break;
