  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    if(do_stats) {
      caffe_net->RangeInLayers(&layer_names_, &max_in_, &max_out_,
          &max_params_,&max_params_vec_);
    }

    // "debugOutput , check max_params_vec_"
    // vector<float> it_temp;
    // for(int i_num = 0; i_num < max_params_vec_.size();i_num++){
    //   it_temp = max_params_vec_[i_num];
    //   for(int j_num = 0; j_num < it_temp.size(); j_num++){
    //     LOG(INFO) << "debugOutput " <<"i_num = " << i_num << " j_num: "<< j_num << "max_params " <<it_temp[j_num];
    //   }
    // }
...
void Quantization::Quantize2DynamicFixedPoint() {
  ...
  for(int i_num = 0; i_num < max_params_vec_.size();i_num++){
    vector<int> it_temp;
    for(int j_num = 0; j_num < max_params_vec_[i_num].size(); j_num++){
      it_temp.push_back((int)ceil(log2(max_params_vec_[i_num][j_num])+1));
      //LOG(INFO) << "debugOutput " <<"i_num = " << i_num << " j_num: "<< j_num << " max_params_vec_: " <<max_params_vec_[i_num][j_num] << " il: " <<(int)ceil(log2(max_params_vec_[i_num][j_num])+1);
    }
    il_params_vec_.push_back(it_temp);
  }
  // for (int i = 0; i < layer_names_.size(); ++i) {
  //     //if(layer_names_[i].find("scale") != string::npos){
  //       LOG(INFO) << "debugOutput " <<" layer_names_ = " << layer_names_[i];
  //       for(int i_num = 0; i_num < il_params_vec_[i].size();i_num++){
  //         LOG(INFO) << "debugOutput " <<" dim= "<<i_num<< " il= "<< il_params_vec_[i][i_num];
  //      }
  //   //}
  // }
  ...
  // for(int i_num = 0; i_num < il_params_vec_.size();i_num++){
  //   for(int j_num = 0; j_num < max_params_vec_[i_num].size(); j_num++){
  //     LOG(INFO) << "debugOutput " <<"layer_names_ = " << layer_names_[i_num] << " il_params_vec_: "<< il_params_vec_[i_num][j_num];
  //   }
  // }
  ...
  //   LOG(INFO) << "debugOutput"<<"EditNetDescriptionDynamicFixedPoint Convolution Parameters";
  ...
  // for (int bitwidth = 16; bitwidth > 0; bitwidth /= 2) {
  //   LOG(INFO) << "debugOutput"<<"EditNetDescriptionDynamicFixedPoint InnerProduct Parameters";
  ...
  // // Score net with dynamic fixed point batch normlization.
  // // The rest of the net remains in high precision format.
  // caffe::ReadNetParamsFromTextFileOrDie(model_, &param);
  // param.mutable_state()->set_phase(caffe::TEST);
  // vector<int> test_bw_bn_params;
  // vector<float> test_scores_bn_params;
  // for (int bitwidth = 16; bitwidth > 0; bitwidth /= 2) {
  //   LOG(INFO) << "debugOutput"<<"EditNetDescriptionDynamicFixedPoint BatchNorm Parameters";
  //   EditNetDescriptionDynamicFixedPoint(&param, "BatchNorm", "Parameters",
  //       -1, -1, bitwidth, -1, -1, -1);
  //   net_test = new Net<float>(param, NULL);
  //   net_test->CopyTrainedLayersFrom(weights_);
  //   RunForwardBatches(iterations_, net_test, &accuracy);
  //   test_bw_bn_params.push_back(bitwidth);
  //   test_scores_bn_params.push_back(accuracy);
  //   delete net_test;
  //   if ( accuracy + error_margin_ / 100 < test_score_baseline_ ) break;
  // }

  // Score net with dynamic fixed point scale.
  // The rest of the net remains in high precision format.
  caffe::ReadNetParamsFromTextFileOrDie(model_, &param);
  param.mutable_state()->set_phase(caffe::TEST);
  vector<int> test_bw_scale_params;
  vector<float> test_scores_scale_params;
  for (int bitwidth = 16; bitwidth > 0; bitwidth /= 2) {
    LOG(INFO) << "debugOutput"<<"EditNetDescriptionDynamicFixedPoint Scale Parameters";
    EditNetDescriptionDynamicFixedPoint(&param, "Scale", "Parameters",
        -1, -1, -1, bitwidth, -1, -1);
    net_test = new Net<float>(param, NULL);
    net_test->CopyTrainedLayersFrom(weights_);
    RunForwardBatches(iterations_, net_test, &accuracy);
    test_bw_scale_params.push_back(bitwidth);
    test_scores_scale_params.push_back(accuracy);
    delete net_test;
    if ( accuracy + error_margin_ / 100 < test_score_baseline_ ) break;
  }
  ...
  for (int bitwidth = 16; bitwidth > 0; bitwidth /= 2) {
    LOG(INFO) << "debugOutput"<<"EditNetDescriptionDynamicFixedPoint all Activations";
    EditNetDescriptionDynamicFixedPoint(&param, "Convolution_and_InnerProduct_and_BatchNorm_and_Scale",
        "Activations", -1, -1, -1, -1, bitwidth, bitwidth);
    net_test = new Net<float>(param, NULL);
    net_test->CopyTrainedLayersFrom(weights_);
    RunForwardBatches(iterations_, net_test, &accuracy);
  ...
  //     break;
  // }
  // for (int i = 0; i < test_scores_bn_params.size(); ++i) {
  //   if (test_scores_bn_params[i] + error_margin_ / 100 >=
  //         test_score_baseline_)
  //     bw_bn_params_ = test_bw_bn_params[i];
  //   else
  //     break;
  // }
  for (int i = 0; i < test_scores_scale_params.size(); ++i) {
    if (test_scores_scale_params[i] + error_margin_ / 100 >=
          test_score_baseline_)
      bw_scale_params_ = test_bw_scale_params[i];
    else
      break;
  }
  ...
  param.mutable_state()->set_phase(caffe::TEST);
  EditNetDescriptionDynamicFixedPoint(&param, "Convolution_and_InnerProduct_and_BatchNorm_and_Scale",
      "Parameters_and_Activations", bw_conv_params_, bw_fc_params_, bw_bn_params_, bw_scale_params_,
        bw_in_, bw_out_);
  net_test = new Net<float>(param, NULL);
  net_test->CopyTrainedLayersFrom(weights_);
  RunForwardBatches(iterations_, net_test, &accuracy);
  ...
  // }
  // LOG(INFO) << "Dynamic fixed point BN";
  // LOG(INFO) << "weights: ";
  // for (int j = 0; j < test_scores_bn_params.size(); ++j) {
  //   LOG(INFO) << test_bw_bn_params[j] << "bit: \t" << test_scores_bn_params[j];
  // }
  LOG(INFO) << "Dynamic fixed point Scale";
  LOG(INFO) << "weights: ";
  for (int j = 0; j < test_scores_scale_params.size(); ++j) {
    LOG(INFO) << test_bw_scale_params[j] << "bit: \t" << test_scores_scale_params[j];
  }
  ...
  // LOG(INFO) << bw_bn_params_ << "bit BN weights,";
  LOG(INFO) << bw_scale_params_ << "bit Scale weights,";
...
void Quantization::EditNetDescriptionDynamicFixedPoint(NetParameter* param,
      const string layers_2_quantize, const string net_part, const int bw_conv, 
        const int bw_fc, const int bw_bn, const int bw_scale, const int bw_in, const int bw_out) {
  for (int i = 0; i < param->layer_size(); ++i) {
    ...
    if (layers_2_quantize.find("Convolution") != string::npos &&
      ...
      if (net_part.find("Parameters") != string::npos) {
        ...
        vector<int> il_vec_temp =  GetIntegerLengthParamsVector(param->layer(i).name());
        // param_layer->mutable_quantization_param()->set_fl_params(bw_conv -
        //     GetIntegerLengthParams(param->layer(i).name()));
        param_layer->mutable_quantization_param()->set_fl_params(bw_conv - il_vec_temp[0]);
        param_layer->mutable_quantization_param()->set_bw_params(bw_conv);      
        // LOG(INFO)  << "debugOutput " << "il_vec size= " << il_vec.size() << " il_= " <<il_vec[0] << " il_params= "<< GetIntegerLengthParams(param->layer(i).name());
        // if(il_vec[0] != GetIntegerLengthParams(param->layer(i).name())){
        //    LOG(INFO)  << "debugOutput " << "FalseFalseFalseFalseFalseFalseFalseFalseFalse ";
        // }
      }
    ...
    if (layers_2_quantize.find("Scale") != string::npos &&
      ...
      if (net_part.find("Parameters") != string::npos) {
        ...
        // param_layer->mutable_quantization_param()->set_fl_params(bw_conv -
        //     GetIntegerLengthParams(param->layer(i).name()));
        param_layer->mutable_quantization_param()->set_fl_params(bw_scale - il_vec_temp[0]);
        param_layer->mutable_quantization_param()->set_bw_params(bw_scale);      
        // LOG(INFO)  << "debugOutput " << "il_vec size= " << il_vec.size() << " il_= " <<il_vec[0] << " il_params= "<< GetIntegerLengthParams(param->layer(i).name());
        // if(il_vec[0] != GetIntegerLengthParams(param->layer(i).name())){
        //    LOG(INFO)  << "debugOutput " << "FalseFalseFalseFalseFalseFalseFalseFalseFalse ";
        // }
    ...
    if (layers_2_quantize.find("BatchNorm") != string::npos &&
        param->layer(i).type().find("BatchNorm") != string::npos) {
      // quantize parameters
      if (net_part.find("Parameters") != string::npos) {
        LayerParameter* param_layer = param->mutable_layer(i);
        param_layer->set_type("BatchNormRistretto");
        vector<int> il_vec_temp =  GetIntegerLengthParamsVector(param->layer(i).name());
        // param_layer->mutable_quantization_param()->set_fl_params(bw_conv -
        //     GetIntegerLengthParams(param->layer(i).name()));
        param_layer->mutable_quantization_param()->set_fl_mean(bw_bn - il_vec_temp[0]);
        param_layer->mutable_quantization_param()->set_bw_mean(bw_bn);
        param_layer->mutable_quantization_param()->set_fl_var(bw_bn - il_vec_temp[1]);
        param_layer->mutable_quantization_param()->set_bw_var(bw_bn);
        param_layer->mutable_quantization_param()->set_fl_mov(bw_bn - il_vec_temp[2]);
        param_layer->mutable_quantization_param()->set_bw_mov(bw_bn);
        // LOG(INFO)  << "debugOutput " << "il_vec size= " << il_vec.size() << " il_= " <<il_vec[0] << " il_params= "<< GetIntegerLengthParams(param->layer(i).name());
        // if(il_vec[0] != GetIntegerLengthParams(param->layer(i).name())){
        //    LOG(INFO)  << "debugOutput " << "FalseFalseFalseFalseFalseFalseFalseFalseFalse ";
        // }
...
vector<int> Quantization::GetIntegerLengthParamsVector(const string layer_name) {
  int pos = find(layer_names_.begin(), layer_names_.end(), layer_name)
      - layer_names_.begin();
      // for(int num_i=0;num_i<il_params_vec_[pos].size();num_i++){
      //   LOG(INFO)  << "debugOutput " << "il_params_vec_:";
      //   LOG(INFO)  << "debugOutput " <<"layer_names_ = " << layer_names_[num_i] << " il_params_vec_:  " << num_i << "  il_= " << il_params_vec_[pos][num_i];
      // }
  return il_params_vec_[pos];
}
