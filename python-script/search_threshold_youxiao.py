...
def binarySearch(minThreshold,maxThreshold):
	flac = 10**5
	n = (float(minThreshold)+float(maxThreshold))/2.00000
	b = math.floor(n*flac)
	return b/float(flac)
...
def test_accuracy(solver_t):
	...
	return accuracy_sum/float(200.0)
...
def init_testnet_dist(origal_net,solver_net,bound):
	...
	n = lenv
	#if(os.path.exists(output_dir+'gaussion.txt')):
	#		os.remove(output_dir+'gaussion.txt')
	#f=open(output_dir+'gaussion.txt','a')
	for k,v in origal_net.params.items():
		z=3
		params_dim = 0
		...
		#f.write('\''+str(k)+'\': '+str(0.4*layer_std+layer_mean)+' ,\\'+'\n')
		for i in v :
			print 'k: ',k,' params_dim: ',params_dim
			solver_net.test_nets[0].params[k][params_dim].data[...] = i.data
			params_dim += 1
...
for k,v in reversed(net.params.items()):
	...
	weight_recover(net.params[k],solver.test_nets[0].params[k],bound_bottom)
	if(accuracy_bottom < accuray_based - 0.4):
		layer_threshold[k] = layer_bound[k][0]
		continue
	if(accuracy_top >= accuray_based - 0.4):
