...
for k,v in net.params.items():
		...
		if (i.data.ndim!=1 and i.data.ndim!=2):
		 		...
		 		for input_num in range(i.data.shape[1]):
		 			for height in range(i.data.shape[2]): 
		 				for width in range(i.data.shape[3]):
		 					...
		 					if(abs(i.data[kernel][input_num][height][width] == 0)):
 								zero_exited += 1
		 					if(abs(i.data[kernel][input_num][height][width])<0.007):
			...
			for neturalUnit_num in range(i.data.shape[0]):
 				for map_num in range(i.data.shape[1]):
 					...
 					if(abs(i.data[neturalUnit_num][map_num] == 0)):
 						zero_exited += 1
 					if(abs(i.data[neturalUnit_num][map_num])<0.007):
 						i.data[neturalUnit_num][map_num] = 0
 						layer_pruned += 1
 		else:
 			print 'layer:',k,i.data.shape,'dim:',i.data.ndim
 			for neturalUnit_num in range(i.data.shape[0]):
 				if(abs(i.data[neturalUnit_num] == 0)):
 					zero_exited += 1
 				if(abs(i.data[neturalUnit_num])<0.007):
	...
	print ('%s :rate = : %.2f ' % (k,((layer_pruned/float(layer_param_num[k]))*100)))
