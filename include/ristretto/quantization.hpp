...
private:
  ...
  void EditNetDescriptionDynamicFixedPoint(caffe::NetParameter* param,
      ...
      const int bw_conv, const int bw_fc, const int bw_bn, const int bw_scale,const int bw_in, const int bw_out);
  /**
  ...
  /**
   * @brief Find the integer length vector for dynamic fixed point parameters of a
   * certain layer.
   */
  vector<int> GetIntegerLengthParamsVector(const string layer_name); 
  /**
  ...
  vector<vector<float> > max_params_vec_;
  vector<vector<int> > il_params_vec_;
  ...
  int bw_bn_params_,bw_scale_params_;
